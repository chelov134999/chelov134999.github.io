...
function updateProgressUI(percent, etaSeconds) {
  ...
  const label = `資料收集中… 進度 ${safePercent}%`;
  ...
  const etaLabel = etaSeconds != null
    ? `預估完成 ${Math.max(0, Math.round(etaSeconds))} 秒`
    : '預估完成時間 60 秒內';
...
function animateFrontProgress(targetPercent, duration = TRANSITION_DURATION_MS) {
  ...
  const step = (now) => {
    ...
    const current = startPercent + (targetPercent - startPercent) * ratio;
    ...
    if (ratio < 1 && state.stage !== 's4' && state.stage !== 's5') {
      ...
      state.progress.frontPercent = Math.max(state.progress.frontPercent, targetPercent);
    }
  };
...
function startMessageTicker() {
  ...
  state.progress.messageId = setInterval(() => {
    ...
    if (tick > state.progress.percent) {
      updateProgressUI(tick);
    }
...
async function handleLeadSubmit(event) {
  ...
  if (!lead.city || !lead.route || !lead.number || !lead.name) {
    showToast('請完整填寫四個欄位');
    return;
  }
...
function handleStatusResponse(payload) {
  ...
  const { stage, percent, eta_seconds: etaSeconds, report } = payload;
  if (typeof percent === 'number') {
    const mergedPercent = Math.max(state.progress.frontPercent, percent);
    updateProgressUI(mergedPercent, etaSeconds);
  }
...
function resetFlow() {
  ...
  state.quiz = { goal: '', tone: [], competitorsInput: [] };
  state.progress = {
    startAt: 0,
    percent: 0,
    frontPercent: 0,
